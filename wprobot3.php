 ...
 */
/*
Plugin Name: WPRobot3
Plugin URI: http://wordpress.org/extend/plugins/WPRobot3/
	...
	if(is_array($installed_modules)) { 
		foreach($wpr_loadedmodules as $module) {
			if(!in_array($module, $installed_modules)) {
				if($module != "translation" && $module != "articlebuilder") {
					$newmodules[] = $module;
				}
			}
		}
...
function wpr_post($camp_id, $keyword="", $retry=1, $manual=0) {
	...
	if($options['wpr_rewrite_active'] == "Yes" && $yahoocat["rw"] == 1) {
		...
		if(empty($options['wpr_sr_rewrite_pw']) || empty($options['wpr_sr_rewrite_email'])) {
			$esql = "INSERT INTO ".$wpr_table_errors." ( campaign, keyword, module, reason, message, time ) VALUES";
			$esql .= " ( '$camp_id', '$errkeyword', 'SpinRewriter', 'SpinRewriter', 'You need to enter your SpinRewriter username and password in the Options!', '$time' );";
			$results = $wpdb->query($esql);		
		} else {
			//echo "<br/><br/>-------orig-------------<br/><br/><div>".$content . "</div><br/><br/>---------rewrite----------<br/><br/>";//////////
